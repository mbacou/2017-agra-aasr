...
# Save for SpatialDev
shapefile(g2[g2$svyCode=="gha-glss6",], "~/Data/GHA-GLSS6/maps/gha-glss6-glss5_L2.shp")
...
# With (300, 5998) clusters.
# svydesign(~clust + hhid, strata = ~rural, w = ~weight, nest = T, 
#     data = gha[survey == "1998/99"])
# Probabilities:
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# 0.0002348 0.0012440 0.0016620 0.0019600 0.0022330 0.0111600 
...
svymean(~I(noaggross_sh*100), subset(gha.svy.farm, !is.na(croparea)), na.rm=T)
#                         mean     SE
# I(noaggross_sh * 100) 22.044 0.0127
# I(noaggross_sh * 100) 17.446 1.1725

svyquantile(~I(noaggross_sh*100), gha.svy.farm, .5, na.rm=T)
#                             0.5
# I(noaggross_sh * 100) 0.8265929

svyby(~I(noaggross_sh*100), ~croparea_4ha, gha.svy.farm, svymean, na.rm=T)
#         croparea_4ha I(noaggross_sh * 100)       se
# <= 4 ha      <= 4 ha              17.58628 1.190880
# > 4 ha        > 4 ha              12.53712 3.285989

svyby(~I(noaggross_sh*100), ~croparea_4ha, gha.svy.farm, svyquantile, .5, ci=T, na.rm=T)
#         croparea_4ha I(noaggross_sh * 100)         se
...
# Save workspace
rm(tmp, tmp.lbl, i, x, tmpha, tmphb, tmpr, tmp1, tmp2, summaryname, svyCrossTab)
...
### Classification of Small and Medium Farm Holdings
...
tmp <- svymean(~I(interaction(cropsales_clas, naggross_clas)), subset(gha.svy[[1]], croparea_4ha=="<= 4 ha"), na.rm=T)
tmp.cv <- confint(tmp)
...
table_options(HTMLcaption="(#tab:tab1) Est. Proportions of Farm Holdings below 4 ha across Categories (percent)")
html(tabular(Heading("Income Diversification")*naggross_clas*(mean+confint)~Heading("Crop Commercialization")*cropsales_clas*Heading()*identity,
...
tmp <- svyCrossTab(list(
  ...
  ~I(100*naggross_sh), 
  ~I(100*cropsales_sh)  
  ), ~class9, subset(gha.svy[[1]], croparea_4ha=="<= 4 ha"), quantiles=c(.25,.5,.75))

table_options(HTMLcaption="(#tab:tab2) Est. Characteristics of Farm Holdings below 4 ha across Categories (percent)")
...
table_options(HTMLcaption="(#tab:tab3) Land Assets of Farm Holdings below 4 ha across Categories (ha)")
...
table_options(HTMLcaption="(#tab:tab4) Est. Production, Sales, and Income of Farm Holdings below 4 ha across Categories (Cedis)")
html(tabular(Variable*(Mean+Q25+Q50+Q75)~Heading()*By*Heading()*Stat*Heading()*identity*Format(digits=1, big.mark=",", scientific=F),
  data=tmp), rmarkdown=T)

```

Estimated distributions of key characteristics for farms below 4 ha.

```{r gha-bplot2a, fig.cap="Est. Farm Sizes of Farm Holdings below 4 ha across Categories"}

par(cex=.8)
svyboxplot(landown~class9, subset(gha.svy[[1]], croparea_4ha=="<= 4 ha"),
  ylab="Est. Land Owned (ha)", ylim=c(0,2))

```

```{r gha-bplot2b, fig.cap="Est. Cultivated Area of Farm Holdings below 4 ha across Categories"}

par(cex=.8)
svyboxplot(croparea_imp~class9, subset(gha.svy[[1]], croparea_4ha=="<= 4 ha"),
  ylab="Est. Cultivated Area (ha)", ylim=c(0,4))

```

```{r gha-bplot2c, fig.cap="Est. Gross Farm Income of Farm Holdings below 4 ha across Categories"}

par(cex=.8)
svyboxplot(I(aggross/1000)~class9, subset(gha.svy[[1]], croparea_4ha=="<= 4 ha"),
  ylab="Est. Gross Farm Income ('000 Cedis)", ylim=c(0,8000))

```


```{r gha-scratch, eval=FALSE}
