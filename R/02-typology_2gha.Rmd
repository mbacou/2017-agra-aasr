...
pplot <- function(..., note=NA, alpha=.1) {
  ...
  title(main=sprintf("Missing or null: %4.0f%%", note), 
...
# Save workspace
rm(tmp, tmp.lbl, i, x, tmpha, tmphb, tmpr)
...
```{r gha-bplotsales, fig.cap="Sources of Crop Commercialization (RIGA, IFPRI, author)", fig.height=4.5, dev="png"}

pplot(I(100*totcropsold/totcropprod)~survey+rural, gha, ylim=c(0,100), alpha=.01,
  ylab="Share of Crop Production Sold (%) Source: author",
  note=gha[, 100*sum(is.na(totcropsold/totcropprod)|totcropsold==0)/.N])

pplot(I(100*shcropsold)~survey+rural, gha, ylim=c(0,100), alpha=.01,
  ylab="Share of Crop Production Sold (%) Source: RIGA",
  note=gha[, 100*sum(is.na(shcropsold)|shcropsold==0)/.N])

pplot(I(100*shagprodsold)~survey+rural, gha, ylim=c(0,100), alpha=.01,
