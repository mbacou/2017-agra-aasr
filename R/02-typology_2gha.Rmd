...
# What is the median `croparea` per ha per survey, region and quintiles of
# `totcropprod`? Use that to impute `croparea`.
...
# What is the median `croparea_imp` per survey, per region, and per quintiles of 
# `shagprodsold`? Use that to re-impute `croparea_imp`.
...
gha[, naggross_sh := aggross/totgross]
...
#  13453    500   2401   1313    752   4387   1500    682   3763   2706 
...
#save(g2, gha, gha.lbl, gha.xtra, gha.xtra.lbl, vars, file="./tmp/2017-agra-aasr_GHA_merged.RData")
...
GLSS collect information on the demographic characteristics of the population, their education, health, employment and time use, migration, housing conditions and household agriculture, with a specific focus on **labour force**, with a labour force module expanded in the GLSS6 to include a section on child labour.
...
par(mfrow=c(1,3), cex=.9, font.main=1)
for (i in unique(gha$svyCode)) boxplot(rowSums(cbind(landown, landshare, -landrent), na.rm=T)~rural, 
...
[doco TBD]. Used RIGA share of agricultural products sold instead of crop sales.
