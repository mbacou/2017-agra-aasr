...
plot(svytable(~croparea_4ha+cropsales_clas, gha.svy[[1]]), main=NA, las=1, col=9,
  ylab=NA, xlab="Crop Commercialization 3-class")
plot(svytable(~croparea_4ha+naggross_clas, gha.svy[[1]]), main=NA, las=1, col=36,
  ylab=NA, xlab="Non-Farm Income 3-class")
plot(svytable(~croparea_4ha+class9, gha.svy[[1]]), main=NA, las=1, col=27,
...
tmp <- svymean(~I(interaction(cropsales_clas, naggross_clas)), gha.svy.shf[[1]], na.rm=T)
tmp.cv <- confint(tmp)
...
html(tabular(Heading("Income Diversification")*naggross_clas*(mean+`95% CI`=confint)~Heading("Crop Commercialization")*cropsales_clas*Heading()*identity,
...
tmp <- svyCrossTab(list(
  ...
  ~I(100*naggross_sh), 
  ~I(100*cropsales_sh)  
  ), ~class9, gha.svy.shf[[1]], quantiles=c(.25,.5,.75))

table_options(HTMLcaption="(#tab:tab2) Est. Characteristics of Farm Holdings below 4 ha across Categories (2012/13, percent)")
...
table_options(HTMLcaption="(#tab:tab4) Est. Production, Sales, and Income of Farm Holdings below 4 ha across Categories (2012/13, Cedis)")
html(tabular(Variable*(Mean+Q25+Q50+Q75)~Heading()*By*Heading()*Stat*Heading()*identity*Format(digits=1, big.mark=",", scientific=F),
  data=tmp), rmarkdown=T)
...
par(cex=.8)
svyboxplot(landown~class9, gha.svy.shf[[1]],
...
par(cex=.8)
svyboxplot(croparea_imp~class9, gha.svy.shf[[1]],
...
par(cex=.8)
svyboxplot(I(aggross/1000)~class9, gha.svy.shf[[1]],
