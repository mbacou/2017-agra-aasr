...
RIGA does not provide annual cultivated area, instead we have measures of land owned, land rented out, and land under shared cropping. IFPRI/HarvestChoice produced a cross-panel harmonized variable (the results look a little dubious for GLSS4, see \@ref(tab:tabcultareahh) below). Using land ownership derived from RIGA instead (i.e. $croparea=landown-landrent$) does yield higher proportions of households for GLSS5 but lower for GLSS6.

```{r, tabcultareahh, results="asis"}

tmph <- gha.xtra[, 100*sum(landown>0, na.rm=T)/.N, keyby=.(year, rural=factor(rural, levels=c(1,0), labels=c("rural", "urban")))]
tmpr <- gha[, 100*sum(rowSums(.SD[, .(landown, -landrent)], na.rm=T)>0, na.rm=T)/.N, keyby=.(year, rural)]

tmph[, Source := "HarvestChoice"]
tmpr[, Source := "RIGA"]
tmp <- rbind(tmph, tmpr)
