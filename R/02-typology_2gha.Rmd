...
RIGA aggregates were used to estimate both crop sales and farm vs. non-farm incomes. Generally in the RIGA approach @riga2007doco:
...
- $cropsales_sh = \frac{cropsales}{cropvalue}$ (but can also be derived from $shagprodsold$)
...
- $totgross = aggross + naggross$
- $aggross_sh = frac{aggross}{totgross}$

### Cultivated Area

RIGA does not provide annual cultivated area, instead we have measures of land owned, land rented out, and land under shared cropping. IFPRI/HarvestChoice produced a cross-panel harmonized variable, but the results look dubious for GLSS4 (see \@ref(tab:tabcultarea_hh) below).

```{r, tabcultarea_hh, results="asis"}

tmp <- gha.xtra[, 100*sum(landown>0, na.rm=T)/.N, keyby=.(year, rural=factor(rural, levels=c(0,1), labels=c("urban", "rural")))]

table_options(HTMLcaption="(#tab:tabcultarea_hh) Proportions of sampled households with any cultivated area across survey years (before imputation)")
html(tabular((`Year`=Factor(year))~Heading("Percent oh Hhlds with Cultivated Area")*Factor(rural)*Heading()*V1*Heading()*Format(digits=2)*identity, 
  data=tmp))

```

We propose to impute annual cultivated area using $croparea=landown-landrent$ instead, but this has obvious shortcomings. Using this approach table \@ref(tab:tabcultarea) below shows the proportions of sampled households with any "cultivated area".

```{r, tabcultarea, results="asis"}

tmp <- gha[, 100*sum(rowSums(.SD[, .(landown, -landrent)], na.rm=T)>0, na.rm=T)/.N, keyby=.(year, rural)]

table_options(HTMLcaption="(#tab:tabcultarea) Proportions of sampled households with any cultivated area across survey years (using land area derived from RIGA)")
html(tabular((`Year`=Factor(year))~Heading("Percent oh Hhlds with Cultivated Area")*Factor(rural)*Heading()*V1*Heading()*Format(digits=2)*identity, 
  data=tmp))
