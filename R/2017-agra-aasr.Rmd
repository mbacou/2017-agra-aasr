...
knitr::opts_chunk$set(warning=F, message=F, fig.width=6, fig.height=3, fig.path="../docs/fig/", base.url="../docs", dpi=300, out.height="340px")
...
# Use ggplot2 for charts instead of lattice
# Note that plotly does not work with geom_ecdf()
#ggplotly(p)
theme_paper <- function(...) theme_fivethirtyeight(
  base_family="Roboto Condensed", base_size=8) +
...
# cropsales_sh
p3 <- ggplot(data=ghs[!is.na(croparea_4ha)], aes(cropsales_sh*100, weight=wt_wave2)) + 
...
p4 <- ggplot(data=ghs[farm_hh==T & !is.na(croparea)], aes(cropsales_sh*100, weight=wt_wave2)) + 
...
t3 <- svyby(~farmarea+lstocksold+TLU_total+aggross_sh+aggross, 
    ~cropsales_any, ghs.svy.shf, svyquantile, 2:9/10, ci=T, na.rm=T)
...
Finally within the sub-population of **small farm holdings** we distinguish between the following 3 categories of households based on the level of crop commercialization (market engagement $cropsales\_sh$):

* *<u>Subsistence</u> smallholding (SSHF)*: farms with only own consumption
...
```{r fig1, fig.cap="Farm Size and Cultivated Area"}

# Quick compare `farmarea` and `croparea` 
par(mfrow=c(1,3))
svyhist(~farmarea, ghs.svy.farm, main="Farm Size (ha)", xlim=c(0,10))
...
svyboxplot(farmarea~croparea_4ha, ghs.svy.farm, main="Farm Size (ha)")

```

Subsistence, transition, and commercial small farm holdings are characterized as follows.

```{r fig2, fig.cap="Proportions of Small Farm Holdings", out.width="240px", fig.align="left"}

par(mfrow=c(1,1))
plot(svytable(~croparea_4ha+cropsales_clas, ghs.svy.shf), main=NA)
pander(svytable(~croparea_4ha+cropsales_any, ghs.svy.farm, Ntotal=100), round=T)
pander(svytable(~croparea_4ha+cropsales_clas, ghs.svy.farm, Ntotal=100), round=T)

```


```{r fig3, fig.show="hold"}
...
# Estimated deciles of `cropsales_sh` for smallholders
t1 <- rbind(
  svyquantile(~croparea, ghs.svy.farm, 2:9/10, na.rm=T),
  svyquantile(~cropsales_sh, ghs.svy.shf, 2:9/10, na.rm=T)*100,
  svyquantile(~cropsales_sh, subset(ghs.svy.farm, croparea_4ha=="> 4 ha"), 2:9/10, na.rm=T)*100)
rownames(t1) <- c(
  "Cultivated Area (ha)", 
  "Crop Commercialization (percent, <= 4 ha)",
  "Crop Commercialization (percent, > 4 ha)")
pander(t1, digits=2, caption="Deciles of Farm Households (weighted)")
