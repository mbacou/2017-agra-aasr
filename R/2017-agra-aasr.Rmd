...
svyCrossTab <- function(var, by, design, quantiles=c(.25,.5,.75), labels=NULL) {
  
  # Reshape means
  x <- lapply(var, function(x) {
    m <- svyby(reformulate(x), reformulate(by), design, svymean, na.rm=TRUE)
  ...
  # Reshape quantiles
  y <- lapply(var, function(x) {
    m <- svyby(reformulate(x), reformulate(by), design, svyquantile, quantiles, ci=TRUE, na.rm=TRUE)
    m <- as.data.table(ftable(m))
    m[, var := x]
    return(m)
  })
  ...
  x <- x[y]
  x[, stat := as.character(stat)]
  x[, var := as.character(var)]
  #x[-seq(1, nrow(x), uniqueN(by)*2), var := NA]
  #x[-seq(1, nrow(x), 2), by := NA]
  setcolorder(x, c(3,1,2,4:ncol(x)))
  setnames(x, 1:3, c("Variable", "By", "Stat"))
  
  # Add optional labels
  if(!missing(labels)) for (i in seq_along(var)) x[Variable==var[i], Variable := labels[i]]
...
svyCrossPlot <- function(formula, breaks="Sturges", design, bandwidth=NULL, ...) {
  par(mfrow=c(3,2))
  for (i in levels(ghs$cropsales_clas)) {
    svyhist(formula, subset(design, cropsales_clas==i), breaks=breaks,
      ylab="Density", main=i, ...)
    lines(svysmooth(formula, subset(design, cropsales_clas==i), bandwidth=bandwidth), lwd=1, col="red")
...
# Subset survey sample to farm and SH farms
...
#               total     SE
# farm_hhFALSE 13.480 325524
# farm_hhTRUE  19.256 270388

svymean(~farm_hh, ghs.svy, na.rm=T)
#                 mean     SE
# farm_hhFALSE 0.41177 0.0082
# farm_hhTRUE  0.58823 0.0082
...
```{r fig1, fig.cap="Fig. Cultivated Area and Crop Commercialization"}
...
svyhist(~cropsales_sh, ghs.svy.farm, main="Crop Commercialization (percent)")
lines(svysmooth(~cropsales_sh, ghs.svy.farm, bandwidth=.1), lwd=1, col="red")
svyboxplot(I(cropsales/1000)~croparea_4ha, ghs.svy.farm, main="Crop Sales ('000 LCU)", ylim=c(0,5E2))
...
tmp <- svyCrossTab(c("cropsales_sh", "cropsales"), "croparea_4ha", 
  ghs.svy.farm, quantiles=c(.25,.5,.75))
tmp[Variable=="cropsales_sh", names(tmp)[4:7] := lapply(.SD, "*", 100), .SDcols=4:7]
tmp[Variable=="cropsales", names(tmp)[4:7] := lapply(.SD, "/", 1000), .SDcols=4:7]

pander(tabular(
  Factor(Variable)*(Mean+Q25+Q50+Q75)~Heading()*Factor(By)*Heading()*Factor(Stat)*Heading()*Format(big.mark=",", nsmall=2, digits=0)*identity,
  data=tmp),
...
pander(tabular(
  ((`Farm Size`=Factor(rn))~Heading()*identity*(Heading("Crop Sales")*(sales+`no sales`)+Heading("Category")*(SSHF+TSHF+CSHF))),
  data=tmp),
  digits=2, big.mark=",", missing=".",
...
```{r fig3, fig.cap="Fig. Distribution of Farm Area (ha)", fig.height=6, out.height="300px"}
...
```{r fig4, fig.cap="Fig. Distribution of Livestock Holdings (TLU)", fig.height=6, out.height="300px"}
...
```{r fig5, fig.cap="Fig. Distribution of Livestock Sales ('000 LCU)", fig.height=6, out.height="300px"}

# lstocksold
svyCrossPlot(~I(lstocksold/1000), ghs.svy.shf, breaks=20, bandwidth=100, xlim=c(0,3000),
  xlab="Livestock Sales ('000 LCU)")

```

```{r fig6, fig.cap="Fig. Share of Farm Income (percent)", fig.height=6, out.height="300px"}

# aggross_sh
svyCrossPlot(~I(aggross_sh*100), ghs.svy.shf, breaks=20, bandwidth=1,
  xlab="Share of Farm (percent)")
...
tmp <- svyCrossTab(c("cropvalue",
  "cropsales", "lstocksold", "agsales", "agsalespc", "aggross"),
  "cropsales_clas", ghs.svy.shf, quantiles=c(.25,.5,.75))
tmp[, names(tmp)[4:7] := lapply(.SD, "/", 1000), .SDcols=4:7]

pander(tabular(
  Factor(Variable)*(Mean+Q25+Q50+Q75)~Heading()*Factor(By)*Heading()*Factor(Stat)*Heading()*Format(digits=0, big.mark=",")*identity,
  data=tmp),
  emphasize.italics.cols=c(4,6,8), 
  caption="Tab. Farm Household Characteristics across Categories ('000 LCU)")

tmp <- svyCrossTab(c("farmarea", "croparea", "TLU_total", "cropsales_sh", "aggross_sh"),
  "cropsales_clas", ghs.svy.shf, quantiles=c(.25,.5,.75))
tmp[Variable %in% c("cropsales_sh", "aggross_sh"), names(tmp)[4:7] := lapply(.SD, "*", 100), .SDcols=4:7]

pander(tabular(
  Factor(Variable)*(Mean+Q25+Q50+Q75)~Heading()*Factor(By)*Heading()*Factor(Stat)*Heading()*Format(digits=0, big.mark=",", nsmall=2)*identity,
  data=tmp),
  emphasize.italics.cols=c(4,6,8), 
  caption="Tab. Farm Household Characteristics across Categories (ha, TLU, percent)")
