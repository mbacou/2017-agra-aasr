...
print(spplot(qd.usgs, "code", col.regions=qd.usgs.lbl[order(code), col], 
  xlab=list("Land Cover Classification\n(USGS/EROS 2013)", cex=.9)), 
  split=c(1,1,2,1), more=T)

print(spplot(qd.suit, col.regions=c(qd.agpot.lbl[c(2,6), col], "grey"), 
  xlab=list("Classified Low/High Agricultural Potential\n(BMGF/SDI 2016)", cex=.9)),
  split=c(2,1,2,1), more=F)
...
kable(qd.usgs.lbl[, .(Code=value, Label=code, Class=class_name)],
  caption="USGA/EROS Land Cover Classification")
...
plot(svytable(~croparea_4ha+suit_bmgf, gha.svy[["gha6"]]), main=NA, las=1, col=rev(viridis(3)),
  ylab=NA, xlab="Crop Suitability 3-class")
plot(svytable(~croparea_4ha+seg_mkt, gha.svy[["gha6"]]), main=NA, las=1, col=viridis(2),
  ylab=NA, xlab="Market Access 2-class")
plot(svytable(~croparea_4ha+seg_quad_4, gha.svy[["gha6"]]), main=NA, las=1, col=viridis(4)[c(1,2,3,4)],
...
```{r gha-class5-fig, fig.cap="Est. Proportions of Farm Holdings across Combined Categories (below/above 4 ha)"}

# Use `seg_quad_4` and a new 5-class typology `class5`
par(mfrow=c(1,2), mar=c(3,0,0,.25), mai=c(.5,0,0,0), cex=1.2, cex.lab=.7)
plot(svytable(~seg_quad_4+cropsales_clas, gha.svy.shf[["gha6"]]), main=NA, las=2, col=viridis(3),
  ylab=NA, xlab="Crop Commercialization")
plot(svytable(~seg_quad_4+naggross_2clas, gha.svy.shf[["gha6"]]), main=NA, las=2, col=viridis(3),
  ylab=NA, xlab="Non-Farm Income low/high")

par(mfrow=c(1,1), mar=c(3,0,0,.25), mai=c(.5,0,0,0), cex=1.2, cex.lab=.7)
plot(svytable(~seg_quad_4+class5, gha.svy.shf[["gha6"]]), main=NA, las=2, col=viridis(5),
  ylab=NA, xlab="Typology 5-class")
