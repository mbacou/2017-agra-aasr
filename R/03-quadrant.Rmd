...
qd.agpot.lbl <- data.table(code=qd.agpot.lbl, label=names(qd.agpot.lbl), col=pal.agpot)
rm(pal.agpot)
...
levels(qd.agpot) <- data.frame(ID=1:9, category=qd.agpot.lbl$label) 
...
# Save workspace
rm(tmp, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, x, i)
...
```{r, fig.cap="Travel Time to Nearest 20K and 50K Market. Source: IFPRI/HarvestChoice, 2016."}
...
print(spplot(g2[g2$svyCode=="gha-glss6",], "seg_mkt", col="grey20", lwd=.2, col.regions=viridis(2),
...
print(spplot(g2[g2$svyCode=="gha-glss5",], "seg_mkt", col="grey20", lwd=.2, col.regions=viridis(2),
...
plot(svytable(~croparea_4ha+seg_mkt, gha.svy[["gha6"]]), 
...
tmp <- svymean(~I(interaction(suit_bmgf, seg_mkt)), gha.svy.shf[["gha6"]], na.rm=T)
tmp.cv <- confint(tmp)
...
tmp[, `:=`(
  ...
  seg_mkt = factor(rep(levels(gha$seg_mkt), each=3, 1))
  )]

table_options(HTMLcaption="(#tab:tab1) Est. Proportions of Farm Holdings below 4 ha across Quadrants (2012/13, percent)")
html(tabular(Heading("Ag. Potential")*suit_bmgf*(mean+CI)~Heading("Market Access")*seg_mkt*Heading()*identity,
...
In this section we combine results from the farm typology developed in [Section 3](./typology-of-farm-holdings.html) with the spatial dimensions outlined above (agricultural suitability and market access).
...
res(qd.tt08)
# [1] 0.008333333 0.008333333 => aleady at 1km
...
# Combine all extracts
g2.dt <- data.table(g2@data,
  tt500k_mean=tmp1[,1], tt500k_med=tmp2[,1], 
  suit_glues_mean=tmp3[,1], suit_glues_med=tmp4[,1])

g2.dt <- data.table(g2@data,
  tt250k_mean=tmp1[,1], tt250k_med=tmp2[,1])
...
summary(g2.dt$tt250k_mean)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.3997  4.0880  5.5510  6.5250  7.7010 42.7300 
summary(g2.dt$tt250k_med)
#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.200   3.400   5.000   5.911   6.800  45.200
...
g2.dt[, seg_suit_glues := factor(suit_glues_mean <= 50, levels=c(T,F), labels=c("agpot-lo", "agpot-hi"))]
...
#     seg_quad_glues_4   N
# 1: agpot-hi / mkt-lo 111
# 2: agpot-lo / mkt-lo  97
# 3: agpot-lo / mkt-hi  65
# 4: agpot-hi / mkt-hi  12
...
# 1: gha-glss6 agpot-lo / mkt-lo 4357
# 2: gha-glss6 agpot-lo / mkt-hi 4903
# 3: gha-glss6 agpot-hi / mkt-lo 6220
# 4: gha-glss6 agpot-hi / mkt-hi 1292
# 5: gha-glss5 agpot-lo / mkt-lo 2439
# 6: gha-glss5 agpot-lo / mkt-hi 2350
# 7: gha-glss5 agpot-hi / mkt-lo 3059
# 8: gha-glss5 agpot-hi / mkt-hi  839
# 9: gha-glss4                NA 5998
...
In this section we combine results from the farm typology developed in [Section 3](./typology-of-farm-holdings.html) with the spatial dimensions outlined above (agricultural suitability and market access using **GLUES suitability** product and travel time to nearest **100K** market).

```{r, fig.cap="GLUES Suitability (Ghana, 2011)"}

include_graphics("fig/glues_suit.png")

```

Ghana's districts classified across areas of lo/hi ag. potential and lo/hi market access (using below/above **4hrs** travel time to nearest **100K** market).

```{r, fig.cap="Spatial Quadrant Segmentation", dev="png", fig.width=7.6}

print(spplot(g2[g2$svyCode=="gha-glss6",], "seg_mkt500k", col="grey60", lwd=.2, col.regions=viridis(2),
  xlab=list("Low/High Market Access\nacross Districts (GLSS6)", cex=.9)),
  split=c(1,1,2,1), more=T)
...
```{r gha-class5glues-fig, fig.cap="Est. Proportions of Farm Holdings across Farm Types and Quadrants (farms below 4 ha, GLUES crop suitability)"}
...
```{r gha-class5glues-tab, results="asis"}
...
table_options(HTMLcaption="(#tab:gha-tab2-class5) Est. Proportions of Farm Holdings below 4 ha across Development Domains and Farm Types (2012/13, percent)")
