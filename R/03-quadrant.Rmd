...
g2.dt[, qd_mkt := factor(tt50k_mean <= 4, levels=c(F,T), labels=c("mkt-lo", "mkt-hi"))]
g2.dt[, quad := paste(suit,ifelse(tt50k_mean <= 4, "mkt-hi", "mkt-lo"), sep=" / ")]
g2.dt[, quad := factor(quad, levels=c(
...
# Map it
g2 <- SpatialPolygonsDataFrame(g2, data.frame(g2.dt), match.ID="rn")
spplot(g2[g2$svyCode=="gha-glss6",], "quad", col="grey20", lwd=.2, col.regions=viridis(6))
...
Below is the resulting cross-classification summarized across Ghana's 170 districts. Low and high market access districts use a **mean travel time to nearest 50K market**.

```{r gha-quadmap, fig.cap="Quadrant Classification across Districts, Ghana 2012/13. Source: authors.", dev="png"}

print(spplot(g2[g2$svyCode=="gha-glss6",], "qd_mkt", col="grey20", lwd=.2, col.regions=viridis(2),
  xlab=list("Low/High Market Access\nacross Districts (GLSS6)", cex=.9)),
  split=c(1,1,2,1), more=T)

print(spplot(g2[g2$svyCode=="gha-glss6",], "quad", col="grey20", lwd=.2, col.regions=viridis(6),
