...
spplot(tmp[[1:5]], col.regions=plasma, maxpixels=100000, at=c(seq(-1,24,1), 62),
  sub="HarvestChoice Travel Times (circa 2007, hrs)",
  par.settings=list(strip.background=list(col="#fafafa")))

png("./docs/fig/HC_tt10.png", width=6, height=5, res=220, units="in", family="Roboto Condensed", pointsize=8)
spplot(mask(qd.tt08[[6:10]], g2[g2$svyCode=="gha-glss6",]),
  col.regions=plasma, maxpixels=300000, at=c(seq(-1,24,1), 62),
  sub=list("HarvestChoice Travel Times (circa 2010, hrs)", font=1),
  par.settings=list(strip.background=list(col="#fafafa")))
dev.off()

# Keep all 2 versions (10 layers) instead.
...
```{r, fig.cap="Sources of hi-res Climatic Constraints for Ghana (e.g. FEWSNET)"}
...
plot(svytable(~seg_quad_4+cropsales_clas, gha.svy.shf[["gha6"]]), 
  main=NA, las=2, cex.axis=1, col=viridis(3),
  ylab=NA, xlab="Crop Commercialization (below 4 ha)")
plot(svytable(~seg_quad_4+naggross_2clas, gha.svy.shf[["gha6"]]), 
  main=NA, las=2, cex.axis=1, col=viridis(3),
  ylab=NA, xlab="Non-Farm Income low/high (below 4 ha)")
plot(svytable(~seg_quad_4+class5, gha.svy.shf[["gha6"]]), 
...
html(tabular(Heading("Development Domain")*seg_quad_4*(mean+CI)~Heading("Farm Type")*class5*Heading()*identity,
  data=tmp), rmarkdown=T, scroll=T, caption="(#tab:gha-tab-class5) Est. Proportions of Farm Holdings below 4 ha across Development Domains and Farm Types (2012/13, percent)")
