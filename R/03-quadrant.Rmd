...
# Also map recent land cover (ESA)
...
qd.lc.lbl <- qd.lc.lbl[J(unique(qd.lc))]
qd.lc <- cut(qd.lc, c(0, unique(qd.lc), maxValue(qd.lc)+1)) # important
qd.lc <- ratify(qd.lc)
levels(qd.lc) <- data.frame(ID=levels(qd.lc)[[1]]$ID, cat=qd.lc.lbl$cat)
spplot(qd.lc, col.regions=qd.lc.lbl$col)

# Map hi-res population density (Worldpop)
...
print(spplot(qd.lc, col.regions=qd.lc.lbl$col,
