...
# Color key from the documentation
qd.agpot.key <- c(
  `very high`=1,
  `high`=2,
  `good`=3,
...
levels(qd.agpot) <- data.frame(ID=1:9, suitability=names(qd.agpot.key)) 
...
```{r gha-suitmap, fig.cap="Crop Suitability for Low-Input Rainfed Cereals. Source: FAO/IIASA/GAEZ.", dev="png"}

print(spplot(qd.agpot, col.regions=pal.agpot, 
  xlab=list("Crop Suitability\nfor Low-Input Rainfed Cereals\n(FAO/IISA/GAEZ)", cex=.9)), 
  split=c(1,1,2,1), more=T)

# print(spplot(g2[g2$svyCode=="gha-glss6", "seg_quad"], col.regions=pal.agpot[c(5,6,3,2)], 
#   xlab=list("Quadrant Classification\nacross Districts (authors)", cex=.9)),
