...
# Plot clusters
fviz_cluster(mod.pam5, geom="point", 
...
save(X, Y, train, 
  mod.pam3, mod.pam5, mod.pam.nb, mod.pam.nb.wss,
  add.alpha, gha, gha.svy, gha.svy.farm,
...
```{r, fig.cap="PAM Clustering of GLSS6 Households (using Euclidean Distance)"}
...
# Plot clusters
fviz_cluster(mod.pam3, geom="point", ggtheme=theme_bw(base_size=10), palette=viridis(3), main="PAM Clusters, k=3")
...
# Plot clusters
fviz_cluster(mod.pam5, geom="point", ggtheme=theme_bw(base_size=10), palette=viridis(5), main="PAM Clusters, k=5")
...
# Compare models
table_options(HTMLcaption="(#tab:gha-clust-pam) Frequency counts of Observations across PAM clusters (k=3 and k=5)")
html(tabular((`PAM k=3`=Factor(clust_pam3))+1~((`PAM k=5`=Factor(clust_pam5))+1)*Format(big.mark=","), 
...
Pair-wise scatter plots across all covariates (cultivated area, sales, and income variables) colored by cluster (k=5).

```{r, dev="png", fig.width=7, fig.height=7}

# Pair-wise plots across clusters
clPairs(train[, .SD, .SDcols=X], train$clust_pam5, 16, add.alpha(viridis(5), .5), CEX=.8)
...
par(mar=c(2.2,0,0,0))
plot(svytable(~clust_pam5+class5, gha.svy.farm[["gha6"]]), 
  main=NA, las=2, cex.axis=1, col=viridis(5),
...
html(tabular((`Cluster`=Factor(clust_pam5))*(mean+CI)~Heading("Proportions")*identity,
