...
fviz_cluster(mod.pam7, geom="point", ggtheme=theme_bw(base_size=10), palette=viridis(7))
...
save(X, Y, train, mod.pam, mod.pam7, mod.pam.nb, mod.agg, mod.div, 
...
Below are pair-wise scatter plots across selected cultivated area and income variables colored by region.
...
mod.pam.nb + geom_vline(aes(xintercept=3), color="grey", linetype="dashed")
...
# Plot clusters
fviz_cluster(mod.pam, geom="point", ggtheme=theme_bw(base_size=10), palette=viridis(3), main="PAM, k=3")

```

```{r, fig.cap="Clusters of Farm Households (PAM, k=7)", dev="png"}

# Plot clusters
fviz_cluster(mod.pam7, geom="point", ggtheme=theme_bw(base_size=10), palette=viridis(7), main="PAM, k=7")
...
html(tabular((`PAM k=3`=Factor(clust_pam))+1~((`PAM k=7`=Factor(clust_pam7))+1)*Format(big.mark=","), 
  data=train))
...
par(mfrow=c(1,3))
for (i in X[1:3]) pplot(as.formula(paste0(i, "~clust_pam")), train, alpha=.02, ylab=NA, xlab=i,
  note=train[, lapply(.SD, function(x) 100*sum(is.na(x)|x==0)/.N), .SDcols=i])

for (i in X[4]) pplot(as.formula(paste0(i, "~clust_pam")), train, alpha=.02, ylab=NA, xlab=i, ylim=c(0, 32000),
  note=train[, lapply(.SD, function(x) 100*sum(is.na(x)|x==0)/.N), .SDcols=i])

for (i in X[5:6]) pplot(as.formula(paste0(i, "~clust_pam")), train, alpha=.02, ylab=NA, xlab=i, ylim=c(0, 8000),
  note=train[, lapply(.SD, function(x) 100*sum(is.na(x)|x==0)/.N), .SDcols=i])
...
We contrast the 2 approaches, cutting the trees at 4 stems.
