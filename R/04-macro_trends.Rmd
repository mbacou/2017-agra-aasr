...
dt.group.imp <- dt.group[, lapply(.SD, na.interpolation, option="linear"), 
  .SDcols=-c(1:2), by=Region]
...
dt.rutt[, yearLabel := ifelse(year==min(year) | year==max(year), year, NA), by=Region]

ggplot(dt.rutt,
  aes(labor/1000, land/1000, color=Region, group=Region, label=yearLabel)) +
  geom_path() +
  geom_point(size=.5) + geom_label(label.size=.1, nudge_x=2, nudge_y=6, show.legend=F) +  
  xlab("Ag. Value Added per Worker (constant 2000 '000 US$)") + 
  ylab("Ag. Value Added per Hectare (constant 2000 '000 US$)") +
  scale_color_manual(values=viridis_pal()(6)) +
  theme_bw() + theme(legend.position="bottom")
...
```{r agg-gdp, fig.cap="Regional Trends in Sectoral Value Added (1990-2015) Source: WDI.", fig.height=4, fig.width=6, dev="png"}
